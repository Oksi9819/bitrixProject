...
if ($ratingTemplateName == "" || $ratingTemplateName == "our_catalog")
{
	$ratingTemplateName = COption::GetOptionString("main", "rating_vote_template", $sRatingVoteType == "star"? "star": "standart");
...
$isLikeTemplate = in_array($ratingTemplateName, array("star", "like_graphic", "mobile_like", "like_react"));
if ($isLikeTemplate)
{
	...
	!array_key_exists('USER_VOTE', $arParams))
{
	...
	if (!empty($arComponentVoteResult))
	{
		...
		if (in_array($ratingTemplateName, array("star", "like_graphic", "mobile_like")))
		{
