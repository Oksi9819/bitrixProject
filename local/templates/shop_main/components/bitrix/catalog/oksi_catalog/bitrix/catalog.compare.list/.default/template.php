    ...
    <div id="<?= $idCompareCount; ?>" class="<?= $mainClass; ?> "<?= $style; ?>><?
        ...
        if ($itemCount > 0) {
            ...
            <div class="bx_catalog_compare_count">
                <p><?= GetMessage('CP_BCCL_TPL_MESS_COMPARE_COUNT'); ?>&nbsp;<span
                            data-block="count"><?= $itemCount; ?></span>
                </p>
            ...
            <table class="compare-items" data-block="item-list">
                ...
                <tr data-block="item-row" data-row-id="row<?= $arElement['PARENT_ID']; ?>">
                    ...
                    <td>
                        <noindex><a class="delete-from-compare" href="#" data-href="<?= $arElement["DELETE_URL"] ?>"
    ...
    $(document).ready(function () {
        $('.delete-from-compare').on('click', function () {
            ...
            $.ajax({
                ...
                error: function () {
                    alert('Товар удалить из сравнения не удалось.');
                }
            });
        });
    });
